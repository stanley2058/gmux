...
#[cfg(not(target_os = "macos"))]
#[test]
fn pane_agent_report_methods_are_not_socket_api() {
    let _lock = test_lock();
    let base = unique_test_dir();
    let config_home = base.join("config");
    let runtime_dir = base.join("runtime");
    ...
    let report = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_removed_report","method":"pane.report_agent","params":{{"pane_id":"{}","source":"gmux:pi","agent":"pi","state":"idle"}}}}"#,
            pane_id
        ),
    );
    ...
    let metadata = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_removed_metadata","method":"pane.report_metadata","params":{{"pane_id":"{}","source":"gmux:pi","custom_status":"idle"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(metadata["error"]["code"], "invalid_request");

    let clear = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_removed_clear","method":"pane.clear_agent_authority","params":{{"pane_id":"{}","source":"gmux:pi"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(clear["error"]["code"], "invalid_request");

    let release = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_removed_release","method":"pane.release_agent","params":{{"pane_id":"{}","source":"gmux:pi","agent":"pi"}}}}"#,
            pane_id
        ),
    );
    assert_eq!(release["error"]["code"], "invalid_request");
