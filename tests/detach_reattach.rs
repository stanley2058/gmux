...
#[test]
fn prefix_d_detaches_client_and_server_persists() {
    // In persistence mode, prefix d detaches the client and the server persists.
    // Flow:
    // 1. Start server
    // 2. Connect client, handshake
    // 3. Send prefix key (Ctrl+B) then 'd'
    ...
    // Send prefix key (Ctrl+B = 0x02) then 'd' (detach in persistence mode).
    send_input(&mut stream, &[0x02]).expect("send prefix");

    // Drain any frames generated by entering navigate mode.
    drain_messages(&mut stream);

    send_input(&mut stream, b"d").expect("send detach key");
    ...
    // shortly after the detach key. There may be some frames in
...
fn reattach_after_detach_shows_current_state() {
    ...
    let (version, error) = client_handshake(&mut stream_a, TEST_PROTOCOL_VERSION, 80, 24)
        .expect("handshake should succeed");
    assert_eq!(version, TEST_PROTOCOL_VERSION);
    assert!(error.is_none(), "{:?}", error);
    ...
    let (version, error) = client_handshake(&mut stream_b, TEST_PROTOCOL_VERSION, 80, 24)
        .expect("handshake should succeed");
    assert_eq!(version, TEST_PROTOCOL_VERSION);
    assert!(
        error.is_none(),
...
fn detached_output_preserves_last_attached_pty_size() {
    ...
    let (version, error) = client_handshake(&mut stream, TEST_PROTOCOL_VERSION, 120, 40)
        .expect("handshake should succeed");
    assert_eq!(version, TEST_PROTOCOL_VERSION);
    assert!(error.is_none(), "{:?}", error);
    drain_messages(&mut stream);
