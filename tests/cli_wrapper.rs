...
fn spawn_gmux_with_pane_history(
    config_home: &Path,
    runtime_dir: &Path,
    socket_path: &Path,
) -> SpawnedGmux {
...
fn spawn_named_server(
    ...
    register_spawned_gmux_pid(Some(child.id()));
...
fn run_named_cli_with_socket_override(
    ...
    if let Some(socket_override) = socket_override {
        command.env("GMUX_SOCKET_PATH", socket_override);
    } else {
        command.env_remove("GMUX_SOCKET_PATH");
    }
...
fn parse_cli_json_output(args: &[&str], output: std::process::Output) -> serde_json::Value {
    ...
    serde_json::from_slice(&output.stdout).unwrap_or_else(|err| {
        panic!(
            "failed to parse JSON response for `gmux {}`: {}\nstdout: {}\nstderr: {}",
            args.join(" "),
            err,
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
...
fn help_commands_exit_successfully() {
    ...
    for args in help_cases {
        ...
        assert!(
            output.status.success(),
            "gmux {} failed: status={:?} stdout={} stderr={}",
            args.join(" "),
            output.status.code(),
            String::from_utf8_lossy(&output.stdout),
            String::from_utf8_lossy(&output.stderr)
...
fn named_sessions_use_separate_servers_and_workspace_state() {
    ...
    assert!(alpha_session["socket_path"]
        .as_str()
        .unwrap()
        .ends_with("/sessions/alpha/gmux.sock"));
...
fn server_stop_command_shuts_down_running_server() {
    ...
    let mut gmux = spawn_gmux(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    ...
    let pid = gmux.child.process_id();
    let exit_status = gmux.child.wait().unwrap();
    unregister_spawned_gmux_pid(pid);
...
fn server_stop_then_restart_restores_pane_history() {
    ...
    let mut gmux = spawn_gmux_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    let restarted = spawn_gmux_with_pane_history(&config_home, &runtime_dir, &socket_path);
    wait_for_socket(&socket_path, Duration::from_secs(5));
    wait_for_socket(&client_socket, Duration::from_secs(5));
    ...
    cleanup_spawned_gmux(restarted, base);
}
...
fn wait_agent_status_exits_immediately_when_status_already_matches() {
    ...
    let reported = send_request(
        &socket_path,
        &format!(
            r#"{{"id":"req_cli_immediate_2","method":"pane.report_agent","params":{{"pane_id":"{}","source":"gmux:pi","agent":"pi","state":"idle"}}}}"#,
            pane_id
        ),
    );
