...
fn client_handshake_clamps_small_terminal_size() {
    ...
    let (version, error) = client_handshake(&mut stream, TEST_PROTOCOL_VERSION, 0, 0)
...
fn no_hello_client_closed_within_five_seconds() {
    ...
    let (version, error) = client_handshake(&mut good_stream, TEST_PROTOCOL_VERSION, 80, 24)
