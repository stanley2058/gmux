...
fn unique_test_dir() -> PathBuf {
    let nanos = SystemTime::now()
        ...
        .unwrap_or(0);
    PathBuf::from(format!(
        "/tmp/gmux-autodetect-test-{}-{nanos}",
        std::process::id()
    ))
}
...
/// Spawn `gmux` (no subcommand) — the auto-detect launch path.
fn spawn_gmux_auto(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
...
/// Spawn `gmux --no-session` — the monolithic escape hatch.
fn spawn_gmux_no_session(
    config_home: &Path,
    runtime_dir: &Path,
    api_socket_path: &Path,
) -> SpawnedGmux {
...
/// Running `gmux` with no server present starts a server
...
fn auto_detect_no_server_spawns_server_and_attaches() {
    ...
    // Run `gmux` (no subcommand) — should auto-detect, spawn server, attach as client.
    let gmux = spawn_gmux_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    let client_pid = gmux.child.process_id().expect("client should have PID");
    assert!(
...
/// Running `gmux` with a server already running attaches
...
fn auto_detect_server_running_attaches_directly() {
    ...
    // Run `gmux` (no subcommand) — should detect the running server and attach.
    let client = spawn_gmux_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    ...
    cleanup_spawned_gmux(client, PathBuf::from("/nonexistent"));
    cleanup_spawned_gmux(server, base);
}

/// Socket path resolution is consistent between server and client.
/// Both derive the client socket from the `GMUX_SOCKET_PATH` override,
...
fn auto_detect_socket_path_consistency() {
    ...
    // Run `gmux` with custom socket paths.
    let gmux = spawn_gmux_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
...
/// `gmux --no-session` bypasses server/client and runs
...
fn no_session_flag_runs_monolithically() {
    ...
    // Run `gmux --no-session` — monolithic mode, no server/client.
    let gmux = spawn_gmux_no_session(&config_home, &runtime_dir, &api_socket);
    ...
    // there is only one gmux process.
    let client_pid = gmux.child.process_id().expect("should have PID");
    assert!(
        process_exists(client_pid),
...
/// persists after the client exits, and a new `gmux` can reattach.
#[test]
fn auto_detect_server_persists_and_reattaches() {
    ...
    // Run `gmux` — auto-detect spawns server + attaches client.
    let mut client1 = spawn_gmux_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    // Run `gmux` again — should detect the running server and reattach.
    let client2 = spawn_gmux_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    thread::sleep(Duration::from_millis(500));
    ...
    cleanup_spawned_gmux(client2, PathBuf::from("/nonexistent"));
    cleanup_test_base(&base);
}
...
fn auto_detect_default_socket_path_from_config_dir() {
    ...
    // Don't set GMUX_SOCKET_PATH or GMUX_CLIENT_SOCKET_PATH.
    ...
    let api_socket = config_home.join(app_dir_name).join("gmux.sock");
    let client_socket = config_home.join(app_dir_name).join("gmux-client.sock");
    ...
    cmd.env_remove("GMUX_SOCKET_PATH");
    cmd.env_remove("GMUX_CLIENT_SOCKET_PATH");

    let child = pair.slave.spawn_command(cmd).unwrap();
    register_spawned_gmux_pid(child.process_id());
    drop(pair.slave);
    let server = SpawnedGmux {
        _master: pair.master,
        child,
    };
...
fn auto_detect_writes_client_and_server_logs_to_separate_files() {
    ...
    let spawned = spawn_gmux_auto(&config_home, &runtime_dir, &api_socket, &client_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
    wait_for_socket(&client_socket, Duration::from_secs(10));
    ...
    let client_log = log_dir.join("gmux-client.log");
    let server_log = log_dir.join("gmux-server.log");
    let monolith_log = log_dir.join("gmux.log");
    ...
    assert!(
        ...
        "persistent client logs should not land in gmux.log: {monolith_content}"
    );
...
fn no_session_writes_startup_logs_to_monolith_file() {
    ...
    let spawned = spawn_gmux_no_session(&config_home, &runtime_dir, &api_socket);
    wait_for_socket(&api_socket, Duration::from_secs(10));
...
fn auto_detect_respects_nested_guard_before_auto_attach() {
    ...
    assert!(
        stderr.contains("nested gmux is disabled by default"),
