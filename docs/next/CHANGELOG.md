...
- Starting Gmux with no restored workspaces, or closing the last workspace, now opens a default workspace instead of leaving the client on an empty screen where direct keybindings such as `cmd+n` were shown but ignored. (#366)
...
- Full-screen TUIs such as Neovim now receive resize-generated terminal responses after Gmux internal pane resizes, so grown panes redraw without waiting for extra input. (#471)
...
- Updated rename and close dialogs to use session wording.
...
- Removed user-configurable workspace keybinding entries; session, tab, pane, and pane-panel keybindings remain configurable.
- Renamed structured log events for session-level lifecycle changes from `workspace.*` to `session.*`.
...
### Fixed
- Fixed a Gmux server crash triggered by pane output containing complex Unicode, emoji, or decomposed accent graphemes. Affected sessions could lose running pane processes or crash again after restore if the same saved pane output was replayed. (#453)
...
- Added an opt-in preview update channel with `gmux channel set preview`, `[update].channel`, automated preview manifests, and GitHub prerelease publishing for users who want fixes before stable releases as Gmux transitions toward less frequent, more stable releases.
- Added a remote SSH bridge keepalive fallback. `gmux --remote` now generates a temporary SSH config that includes the user's SSH config first, then adds `ServerAliveInterval` and `ServerAliveCountMax` only when the user has not already configured keepalives. Set `[remote].manage_ssh_config = false` to disable this. (#354, #355, thanks @SunskyXH)
- Added `ui.right_click_passthrough_modifier` so a configured modifier such as `ctrl` can forward right-click hold and drag gestures to mouse-reporting pane apps while normal right-click still opens Gmux's pane menu. (#148)
- Added Kilo Code CLI automatic detection for idle, working, and blocked terminal states. (#270)
- Added `gmux integration install copilot` for GitHub Copilot CLI hooks that report prompt, tool, post-approval progress, permission, `ask_user`, `exit_plan_mode`, idle, session-exit state, and session ids through Gmux's socket API. When native agent session restore is enabled, Gmux can resume Copilot panes with `copilot --resume=<id>`. (#232, #386, thanks @LaneBirmingham)
...
- Claude Code, Codex, and OpenCode integrations now report session identity only. Native state for those agents comes from Gmux's screen detection, while Pi, OMP, GitHub Copilot CLI, Hermes Agent, Qoder CLI, and custom socket integrations can still report state.
...
- Commands launched after `--`, such as `gmux agent start ... -- opencode --session <id>`, now preserve child argv flags instead of parsing them as Gmux flags. (#383)
- Pane apps that request any-motion mouse tracking now receive hover/move events, making Textual-style TUI mouse interaction more reliable inside Gmux. (#419)
...
- Pane apps that query OSC 4 ANSI palette colors now receive the active terminal palette response, so OpenCode and similar TUIs can enable system-theme behavior inside Gmux. (#387)
...
- Pane text selection now derives its highlight colors from the host terminal or active Gmux palette instead of forcing the theme's blue accent. (#298)
- `gmux channel set preview` and `gmux channel set stable` now update direct installs from the selected channel immediately, reject preview on Homebrew and Nix installs before changing config, and show package-manager guidance for managed installs.
- Plain `gmux update` and remote binary replacement now ask before stopping running sessions, avoid protocol-heavy prompt text, and leave the current install untouched when the user chooses not to stop active pane processes. Explicit `--handoff` update flows try live handoff without a second handoff prompt.
- Remote bootstrap now uses the remote shell only for PATH discovery and runs internal probes through `/bin/sh`, so `gmux --remote` can detect existing installs when the remote login shell is fish. (#396)
...
- `gmux session stop` no longer fails on zero-duration socket timeouts when the stop deadline is nearly exhausted.
...
### Added
- Added native agent session restore behind `[session] resume_agents_on_restore`, allowing supported Pi, Claude Code, Codex, OpenCode, and Hermes panes with current official integrations to restart into their previous agent conversation after a Gmux server restart. (#233)
...
- Added `ui.mobile_width_threshold` to configure the terminal width at which Gmux switches to the mobile single-column layout. (#317)
- Added `--handoff` for `gmux update` and `gmux --remote` to opt into live server handoff for supported running servers. Plain update and remote attach use the normal restart/stop flow by default.
- Added `pane.report_metadata` and `gmux pane report-metadata` so user hooks can customize pane titles, displayed agent names, compact status labels, and visible state labels without taking over integration-owned lifecycle or session state. (#36)
...
- Added Ctrl-click URL opening inside panes for OSC 8 hyperlinks and visible `http://` or `https://` URLs when the host terminal sends the modified click to Gmux. (#307)
- Added Qoder CLI detection, terminal state heuristics, and `gmux integration install qodercli` hook support. (#308, #309, thanks @wayneleelwc)

### Fixed
- Remote bootstrap now downloads exact-version release assets for Homebrew and Nix clients instead of copying package-manager-managed local binaries into `~/.local/bin/gmux`.
...
- `gmux session stop` is more reliable when the server closes the socket early or stops without sending a full response.
...
- Linux sound alerts no longer fall back to `aplay` for mp3 files, preventing static noise on systems without `paplay`. Gmux now tries mp3-capable players such as `pw-play`, `ffplay`, `mpg123`, and `mpv` instead. (#290)

## [0.6.2] - 2026-05-23

### Added
- Added optional Nix flake support for building, running, installing, and developing Gmux with Nix. (#208, #221, #264)
- Added `terminal.new_cwd` to choose whether new panes, tabs, and workspaces follow the source pane/workspace, start in `$HOME`, use Gmux's process directory, or use a fixed path.
- Added `gmux integration install omp` for OMP's `.omp` extension directory. The extension reports OMP pane state through Gmux's socket API without relying on native `omp` process detection.
- Added CLI and socket API support for Git worktrees with `gmux worktree list/create/open/remove`, optional worktree provenance on workspace responses, and client/server protocol version 10.
...
- Added `--remote-keybindings local|server` for `gmux --remote`. Remote attach now uses the launching client's local keybindings by default without copying config files to the remote host; use `--remote-keybindings server` to keep the remote server's keybindings. The client/server protocol is now version 9.
...
### Fixed
- Pane apps that query OSC 10/11 default foreground/background colors now receive the host terminal colors, so OpenCode and similar TUIs can detect light terminal themes inside Gmux. (#253)
...
- `gmux --remote` now offers to restart the remote server after installing or replacing a remote binary, or when the running server version differs, even if the client/server protocol is still compatible.
...
- Added `gmux config reset-keys` to back up `config.toml` and remove custom keybindings so built-in v2 defaults apply on restart or config reload. (#154)
- Added an integrations tab in settings and first-run onboarding so users can install recommended agent integrations from inside Gmux.
...
- Added `terminal.default_shell` to choose the executable used for new interactive panes. When unset, Gmux still falls back to `$SHELL`, then `/bin/sh`. (#196)
...
- Running `gmux server` directly now prints socket and log paths and explains that normal TUI users should run `gmux`.
- Kitty graphics virtual Unicode placeholders now render image placements instead of leaving placeholder cells behind. (#136)
- Clipboard image reads are now capped to Gmux's image payload limit, preventing oversized local clipboard images from being read into memory.
- The install script now reads Gmux's public latest-release manifest, so fresh installs use the same binary URLs as `gmux update`.
...
- Remote clients now bridge local clipboard images into the remote pane by staging them as temporary image files and pasting the remote path, so Claude Code image paste works over `gmux --remote`. (#205)

### Breaking Changes
- Removed the separate `keys.quit` binding. Use `keys.detach`, which detaches in server mode and exits in `--no-session` mode. The default detach binding is now `prefix+d`, with `prefix+q` kept as a legacy alias.
...
### Fixed
- The Claude Code integration no longer reports successful or failed post-tool hooks as `working`, and installing the updated integration removes Gmux's deprecated post-tool hook entries from existing Claude settings. (#198)
...
### Added
- Added the `terminal` built-in theme, which uses the host terminal's ANSI palette for Gmux UI colors. (#140, #146, thanks @babymastodon)
...
### Fixed
- Running the internal `gmux client` command from inside Gmux now respects the nested-launch guard, and the command is no longer advertised in root help. (#187)
- The Gmux agent skill now refuses to claim pane ownership unless it is running inside Gmux. (#152)
...
- Gmux now preserves `session.json` symlinks when saving persistent session state. (#139, #147, thanks @cloudmanic)
...
- Remote attach now uses a matching `gmux` already available on the remote `PATH` before installing a new copy. (#170)
...
- Added direct terminal attach with `gmux agent attach <target>` and `gmux terminal attach <terminal_id>`.
...
- Gmux now redraws immediately when the outer terminal regains focus.
...
### Fixed
- PageUp and PageDown now scroll Gmux pane scrollback for normal panes while still forwarding keys to full-screen or mouse-reporting apps.
- Enhanced tilde key sequences now parse correctly, improving compatibility with terminals that emit them.
- `gmux integration install codex` now enables the current Codex `[features] hooks = true` flag and migrates the deprecated top-level `codex_hooks` flag.

### Breaking Changes
- `advanced.allow_nested` has moved to `experimental.allow_nested`; update configs that allow nested Gmux launches.
...
### Added
- Added manual pane labels through `gmux pane rename`, the `pane.rename` socket API, an optional `keys.rename_pane` binding, and the right-click pane menu.
...
- Added `gmux integration status [--outdated-only]` so installed agent integrations can be checked for legacy or outdated versions.
...
### Added
- Added ANSI-formatted pane reads to the CLI and socket API with `gmux pane read --format ansi` / `--ansi`, preserving colors and styles for visible and recent pane output.
...
- `gmux update` now downloads and prepares the new binary before stopping a running server, reducing the chance of interrupting an active session when download or install preparation fails.

## [0.5.6] - 2026-05-09

### Added
- Added the `vesper` built-in theme. (#71, thanks @nexxeln)
- Added `gmux --remote <ssh-target>`, so you can use Gmux as a thin client for remote servers without SSHing in first. Gmux connects over SSH, bootstraps a matching remote `gmux` binary when needed, starts the remote server automatically, and streams an efficient terminal view back to your local terminal.
...
### Fixed
- OSC 8 hyperlinks emitted inside panes now remain clickable after Gmux renders them, including titled markdown-style links.
...
### Added
- Added a mobile layout for narrow terminals, making it practical to SSH into your machine and run gmux from your phone.
...
### Fixed
- Visible active-tab panes that finish while the outer terminal is unfocused are now marked as seen when you return to gmux, preventing stale done/attention indicators.
...
### Added
- Added named persistent sessions, so you can keep separate gmux environments for different projects or contexts while sharing the same global config. See the docs for the full session CLI. (#57, thanks @fbettag)
- Added `gmux status`, `gmux status server`, and `gmux status client` to inspect the local client, running server, protocol compatibility, socket path, and whether a restart is needed.

### Changed
- Focused panes can now still alert you through terminal notifications when the gmux terminal window is unfocused, so active work does not go quiet just because you switched to another app.
...
- `gmux integration install` and `gmux integration uninstall` now work without requiring a running gmux server.
...
### Added
- Config can now be reloaded in the running app/server from the global menu or with `gmux server reload-config`, applying safe live settings without restarting the persistent server.
...
### Added
- Toast notifications can now be delivered through the outer terminal as desktop notifications. Configure this with `ui.toast.delivery = "terminal"`; see the [configuration docs](https://gmux.dev/docs/configuration/) for details.
- Gmux now writes separate capped support logs for app, client, and server modes, making persistent-session issue reports easier to diagnose without unbounded log growth.
...
- In-app toast delivery now stays inside gmux instead of also forwarding a terminal/desktop notification.

## [0.5.0] - 2026-04-21

### Breaking Changes Please Read
- gmux now defaults to a persistent server/client session model. running `gmux` starts or reattaches to a background session server instead of launching the old single-process UI.
- quitting the UI in default mode now detaches the current client and leaves the shared session running. use `gmux server stop` to stop the background server explicitly.
- the old monolithic behavior is still available as an escape hatch with `gmux --no-session`.

### Added
- Persistent sessions are now the default product behavior. You can detach and reattach without stopping pane processes.
- Added the thin client and headless server as first-class product components, including auto-detect launch, explicit `gmux client`, and `gmux server stop`.
...
### Breaking Changes Please Read
- The update flow changes in `0.4.11`. Gmux no longer installs updates silently in the background. Starting with this release, gmux only checks for updates and shows them in the UI. To install a new release, quit gmux and then run `gmux update` manually in your shell.
- This prepares the upcoming `0.5.0` persistence release. Gmux is moving from the old single-binary update model toward a persistent server/client session model, so your workspace can keep running while clients attach, detach, and reconnect.
- The reason for this change is upgrade safety. Gmux needs to stop the old running process cleanly before the new client/server model takes over, so manual update avoids mixed-version states during the transition.

### Added
- Hook-reported agent state can now use custom agent labels, so integrations are no longer limited to gmux’s built-in agent names. Custom labels now flow through pane/workspace UI and the socket API anywhere agent names are shown.

## [0.4.10] - 2026-04-14

### Added
- Prefix mode now supports custom command keybindings via `[[keys.command]]`, so you can launch detached shell helpers or open temporary overlay panes from inside gmux using the active workspace, tab, pane, and cwd context.
...
### Fixed
- Droid's primary-screen redraws no longer erase pane scrollback inside gmux, while normal scrollback-clear behavior is preserved elsewhere.
...
### Added
- Themes can now set `panel_bg = "reset"` to let gmux’s panel chrome inherit the host terminal background instead of painting an opaque panel fill. This also accepts the aliases `default`, `none`, and `transparent`.
...
### Added
- `gmux workspace create` and `gmux tab create` now support `--label`, so scripts and agents can name new workspaces and tabs immediately instead of creating them first and renaming them afterward.
- The global menu now includes a manual **reload keybinds** action, so you can apply `config.toml` keybinding changes without restarting gmux.
- The socket API and CLI now expose a `done` agent status, including `gmux wait agent-status --status done`, so automation can distinguish finished agent runs from panes that are merely idle.

### Changed
- Session state is now saved automatically with a debounce while you work, so recent workspace, tab, pane, and sidebar changes are preserved more reliably even if gmux exits unexpectedly.
...
- `gmux pane run ...` now uses the bracketed-paste-aware input path, improving compatibility with shells and terminal apps that expect pasted command text to arrive atomically.
...
- `gmux pane run ...` now submits the full command atomically in one request, fixing cases where scripted commands did not reliably execute because the final Enter was sent separately.
...
### Major Changes
- Gmux now uses a Ghostty-backed terminal engine as its pane runtime.
...
- Debug builds now use separate runtime/config paths from normal releases, which avoids local development sessions colliding with your main gmux install.

### Fixed
- Starting a second gmux instance against an active socket now fails fast with a clear error instead of clobbering the running session.
...
- Added first-class tab support to the local socket API and CLI wrappers, including `gmux tab ...` commands and tab ids like `1:2` alongside workspace-scoped pane ids.
...
- Added a post-update release-notes screen so gmux can explain what changed after an update is installed.
...
### Changed
- Mouse wheel handling now follows the tmux/Ghostty model more closely: fullscreen apps receive wheel input when they own scrolling, while gmux keeps host scrollback for panes that are behaving like a normal terminal transcript.
- Pane scrollbars now only appear when gmux has real host scrollback for that pane, instead of implying a host-managed scroll position for app-owned scrolling.

### Fixed
- Fixed Codex and pi panes becoming unscrollable in gmux by preserving recoverable host history for top-anchored normal-screen output, without relying on alternate-screen scrollback retention.
- Fixed pane wheel routing so apps using mouse reporting or alternate-scroll behavior can receive scroll input directly instead of having gmux always intercept it.
...
- Fixed alternate-screen scrollback handling so full-screen terminal apps can preserve recoverable history inside gmux panes instead of losing rows that scroll off.
- Fixed Codex in gmux panes losing transcript/history while running in alternate screen, so past output remains scrollable instead of disappearing as the session grows.
...
### Added
- Gmux now checks for updates at startup and periodically while it stays open, so long-running sessions can still discover new releases without a restart cycle.
...
### Added
- Added a local Unix socket API for controlling running gmux sessions, including workspace and pane management, pane reads, text/key input, pane splitting, and output waits.
...
- Added CLI wrappers on top of the socket API with `gmux workspace ...`, `gmux pane ...`, and `gmux wait ...`, using compact public ids like `1` and `1-2` for scripting and agent orchestration.
- Added a settings popup with mouse support for changing themes, sound alerts, and toast notifications from inside gmux.
...
- Gmux now blocks nested launches by default when started inside a gmux-managed pane; set `advanced.allow_nested = true` to opt back in.
...
### Documentation
- Refreshed the README, socket API reference, and reusable agent skill docs to better explain gmux's agent multiplexer model and integration surface.
...
- Added configurable keybindings for all navigate mode actions: new workspace, rename workspace, close workspace, resize mode, and toggle sidebar. See the [configuration docs](https://gmux.dev/docs/configuration/) for the full key reference.
...
### Fixed
- Improved foreground job detection on Linux and macOS so gmux can recognize agents that run through wrapper processes or generic runtimes, including cases like Codex running under `node`.
