...
Seeds a running gmux server with navigator demo workspaces, tabs, panes,
and fake agent states for recording the session navigator.

Environment:
  GMUX_NAV_SOCKET_PATH  API socket to target. Defaults to $HOME/.config/gmux-dev/gmux.sock.
  GMUX_NAV_CWD          Workspace cwd for created panes. Defaults to the repo root.
  GMUX_NAV_BIN          Gmux binary to call. Defaults to cargo run from the repo.
USAGE
}
...
workspace_cwd="${GMUX_NAV_CWD:-$repo_dir}"
config_home="${XDG_CONFIG_HOME:-$HOME/.config}"
dev_socket="$config_home/gmux-dev/gmux.sock"
main_socket="$config_home/gmux/gmux.sock"
export GMUX_SOCKET_PATH="${GMUX_NAV_SOCKET_PATH:-$dev_socket}"

if [[ "$allow_main" != 1 && "$GMUX_SOCKET_PATH" == "$main_socket" ]]; then
  echo "refusing to seed main gmux session: $GMUX_SOCKET_PATH" >&2
  echo "use GMUX_NAV_SOCKET_PATH for a dev socket, or pass --allow-main intentionally" >&2
  exit 1
fi

if [[ ! -S "$GMUX_SOCKET_PATH" ]]; then
  echo "gmux socket not found: $GMUX_SOCKET_PATH" >&2
  echo "start a dev server first, or set GMUX_NAV_SOCKET_PATH" >&2
  exit 1
fi

cd "$repo_dir"

if [[ -n "${GMUX_NAV_BIN:-}" ]]; then
  run() { "$GMUX_NAV_BIN" "$@"; }
else
  run() { cargo run --quiet -- "$@"; }
fi
...
cat <<EOF
Seeded navigator demo data via $GMUX_SOCKET_PATH
