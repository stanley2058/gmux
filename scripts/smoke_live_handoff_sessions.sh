...
GMUX_BIN="${GMUX_BIN:-$ROOT/target/debug/gmux}"
BASE="${BASE:-$(mktemp -d /tmp/gmux-handoff-smoke.XXXXXX)}"
...
cleanup() {
  ...
  for session in "${sessions[@]}"; do
    run_gmux "$session" server stop >/dev/null 2>&1 || true
  done
...
run_gmux() {
  ...
  env -u GMUX_SOCKET_PATH \
    -u GMUX_CLIENT_SOCKET_PATH \
    -u GMUX_SESSION \
    XDG_CONFIG_HOME="$CONFIG_HOME" \
    XDG_RUNTIME_DIR="$RUNTIME_DIR" \
    XDG_STATE_HOME="$STATE_DIR" \
    "$GMUX_BIN" --session "$session" "$@"
}

session_dir() {
  local session="$1"
  if [[ "$session" == "default" ]]; then
    printf '%s/gmux-dev' "$CONFIG_HOME"
  else
    printf '%s/gmux-dev/sessions/%s' "$CONFIG_HOME" "$session"
  fi
}

api_socket() {
  printf '%s/gmux.sock' "$(session_dir "$1")"
}

client_socket() {
  printf '%s/gmux-client.sock' "$(session_dir "$1")"
}

assert_smoke_socket() {
  local socket="$1"
  case "$socket" in
    "$CONFIG_HOME"/gmux-dev/gmux.sock | "$CONFIG_HOME"/gmux-dev/sessions/*/gmux.sock)
      ;;
    *)
...
echo "using gmux: $GMUX_BIN"
echo "smoke base: $BASE"

cargo build --locked --manifest-path "$ROOT/Cargo.toml" >/dev/null
mkdir -p "$CONFIG_HOME/gmux-dev" "$RUNTIME_DIR" "$STATE_DIR"
printf 'onboarding = false\n' > "$CONFIG_HOME/gmux-dev/config.toml"

for session in "${sessions[@]}"; do
  echo "starting smoke session $session at $(api_socket "$session")"
  run_gmux "$session" server >/dev/null 2>&1 &
  server_pids+=("$!")
