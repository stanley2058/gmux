...
impl App {
    ...
    pub(crate) fn handle_scheduled_tasks(&mut self, now: Instant, _geometry_dirty: bool) -> bool {
        let mut changed = false;

        self.sync_animation_timer(now);

        if now >= self.next_resize_poll {
            let resized = self.handle_resize_poll();
            changed |= resized;
...
mod tests {
    ...
    fn git_refresh_deduplicates_workspaces_with_same_cache_key() {
        let repo =
            std::env::temp_dir().join(format!("gmux-git-refresh-dedupe-{}", std::process::id()));
    ...
    fn git_refresh_items_use_cwd_cache_key_for_non_git_cwd() {
        ...
        let cwd = std::env::temp_dir().join(format!("gmux-non-git-cwd-{}", std::process::id()));
