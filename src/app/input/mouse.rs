...
impl AppState {
    ...
    pub(crate) fn pane_scroll_metrics(
        ...
        self.session_index()
            .and_then(|i| self.runtime_for_pane_in_session_at(terminal_runtimes, i, pane_id))
    ...
    pub(super) fn forward_pane_mouse_button(
        ...
        let Some(rt) = self.runtime_for_pane_in_session_at(terminal_runtimes, ws_idx, info.id)
        else {
            return false;
        };
    ...
    pub(super) fn scrollbar_target_at(
        ...
        let rt = self.runtime_for_pane_in_session_at(terminal_runtimes, ws_idx, info.id)?;
...
mod tests {
    ...
    fn absent_toast_does_not_create_clickable_hit_area() {
        ...
        assert_eq!(
            app.state.session_containers[1].focused_pane_id(),
            Some(first_pane)
        );
        assert_eq!(app.state.mode, Mode::Terminal);
    }
    ...
    fn dragging_pane_split_updates_captured_layout_ratio() {
        ...
        app.state.active = Some(0);
        app.state.selected = 0;
        app.state.session_containers[0].test_split(Direction::Horizontal);
        crate::ui::compute_view(&mut app.state, Rect::new(0, 0, 106, 20));
    ...
    fn pane_split_hitbox_does_not_overlap_bottom_pane_content() {
        ...
        app.state.active = Some(0);
        app.state.selected = 0;
        app.state.session_containers[0].test_split(Direction::Vertical);
        crate::ui::compute_view(&mut app.state, Rect::new(0, 0, 106, 20));
        let border = app.state.view.split_borders[0].clone();
    ...
    fn mobile_switcher_swallows_non_left_mouse_events() {
        let mut app = app_for_mouse_test();
        app.state.session_containers = vec![Workspace::test_new("one")];
        app.state.active = Some(0);
        app.state.selected = 0;
        app.state.mode = Mode::Terminal;
