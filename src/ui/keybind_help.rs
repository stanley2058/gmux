...
pub(super) fn keybind_help_groups(app: &AppState) -> Vec<HelpGroup> {
    ...
    groups.push((
        ...
        vec![
            ...
            help_entry("enter", "open item"),
            help_entry("1..9", "switch item"),
        ],
    ));
    ...
    let tabs = vec![
        ...
        help_entry(indexed_label(&kb.switch_tab), "switch tab by number"),
        help_entry(keybind_label(&kb.close_tab), "close tab"),
    ];
    groups.push(("tabs", tabs));
    ...
    let panes = vec![
        ...
        help_entry(keybind_label(&kb.view_scrollback), "view scrollback"),
