...
# GMUX_INTEGRATION_ID=qodercli
# GMUX_INTEGRATION_VERSION=1
#
# Reports qodercli agent state changes to gmux. Registered as a Command hook
# in ~/.qoder/settings.json by `gmux integration install qodercli` and
...
# `assets/claude/gmux-agent-state.sh` already consumes claude code's stdin
# payload. No environment variable is consulted for the event identity.

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/gmux-qodercli-hook.XXXXXX")" || exit 0
...
GMUX_ACTION="$action" GMUX_HOOK_INPUT_FILE="$hook_input_file" python3 - <<'PY'
import json
import os
import random
import socket
