"""Hermes plugin installed by Gmux to report agent lifecycle state."""

# GMUX_INTEGRATION_ID=hermes
...
def _base_params() -> tuple[str, str] | None:
    if os.environ.get("GMUX_ENV") != "1":
        return None
    pane_id = os.environ.get("GMUX_PANE_ID", "").strip()
    socket_path = os.environ.get("GMUX_SOCKET_PATH", "").strip()
