...
# GMUX_INTEGRATION_VERSION=5

set -eu

action="${1:-}"
hook_input_file="$(mktemp "${TMPDIR:-/tmp}/gmux-claude-hook.XXXXXX")" || exit 0
...
if hook_event_name == "SubagentStop":
    # SubagentStop is a completion event. Older Gmux integrations mapped it
