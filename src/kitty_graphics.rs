...
pub(crate) fn has_visible_pane_graphics(
    ...
    for info in &app.view.pane_infos {
        let Some(runtime) = app.runtime_for_pane_in_session_at(terminal_runtimes, ws_idx, info.id)
        else {
            continue;
        };
...
fn collect_visible_placements(
    ...
    for info in &app.view.pane_infos {
        let runtime = match app.runtime_for_pane_in_session_at(terminal_runtimes, ws_idx, info.id) {
            Some(rt) => rt,
            None => {
