//! Auto-detect launch behavior for the `gmux` command.
//!
//! When the user runs `gmux` with no subcommand:
...
/// Checks whether a gmux server is currently listening on the client socket.
///
...
/// Checks whether a gmux server is listening at a specific socket path.
...
fn validate_running_server_compatibility() -> io::Result<()> {
    let Some(status) = read_server_status()? else {
        return Err(io::Error::other(format!(
            "a gmux server is listening, but its status API is unavailable.\n\n{}\nIf that fails, stop the old server process manually.",
    ...
    Err(io::Error::other(format!(
        "Gmux was updated, but this session is still running the old server.\n\nserver: v{} protocol {}\nclient: v{} protocol {}\n\n{}",
...
/// Spawns the gmux server as a background daemon process.
///
...
/// - Inherits relevant environment variables (`XDG_CONFIG_HOME`, `GMUX_SESSION`,
...
pub fn spawn_server_daemon() -> io::Result<u32> {
    ...
    let child = command.spawn().map_err(|err: io::Error| {
        io::Error::new(err.kind(), format!("failed to spawn gmux server: {err}"))
    })?;
...
///
/// This is the entry point called from `main.rs` when the user runs `gmux`
...
mod tests {
    ...
    fn server_daemon_command_clears_socket_overrides_for_explicit_session() {
        ...
        std::env::set_var("GMUX_CLIENT_SOCKET_PATH", "/tmp/inherited-client.sock");
        ...
        let command = build_server_daemon_command(PathBuf::from("/tmp/gmux-test"));
        ...
        assert!(envs.iter().any(|(key, value)| {
            *key == OsStr::new("GMUX_CLIENT_SOCKET_PATH") && value.is_none()
        }));
    ...
    fn validate_running_server_compatibility_names_session_commands_for_protocol_mismatch() {
        ...
        assert!(
            message.contains("Run `gmux session stop work`"),
            "unexpected error: {message}"
        );
        assert!(
            message.contains("then run `gmux session attach work` again"),
            "unexpected error: {message}"
        );
